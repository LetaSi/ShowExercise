...
----------------------------------------------------------------------
Modified: Add display option to the library : 
to hide and show the upload control, default: true [Leta Si]
How to use:
 $(function () {
...
if (jQuery) (
	function ($) {
	    ...
	    $.extend($.fn, {
	        fileUpload: function (options) {
	            if (flashVer >= 9) {
	                $(this).each(function () {
	                    ...
	                    // singlton For Client show or hide Uploadfile control and clean queue when switch 
	                    // between show and hide [leta]
	                    if (document.getElementById($(this).attr("id") + 'Uploader') != null) {
	                        if (settings.display == false) { $("#" + $(this).attr('id') + "Uploader").hide(); } else { $("#" + $(this).attr('id') + "Uploader").show(); }
	                        // clean Queue and Queue interface 
	                        $('div.fileUploadQueueItem').remove();
	                        for (var i = 0; i < queueIDs.length; i++) {
	                            $('#' + $(this).attr('id')).fileUploadCancel(queueIDs[i]);
	                        }
	                        return;
	                    }
	                    ...
	                    // hide control for first setting : display --> false  [leta]
	                    if (settings.display == false) {
	                        $("#" + $(this).attr('id') + "Uploader").hide();
	                    }
	                    ...
	                    $(this).bind("rfuSelect", { 'action': settings.onSelect }, function (event, queueID, fileObj) {
	                        if (event.data.action(event, queueID, fileObj) !== false) {
	                            ...
	                            $('#' + $(this).attr('id') + 'Queue').append('<div id="' + $(this).attr('id') + queueID + '" class="fileUploadQueueItem">\
									<div class="cancel">\
										<a href="javascript:$(\'#' + $(this).attr('id') + '\').fileUploadCancel(\'' + queueID + '\',\'' + fileObj.name + '\')"><img  src="' + settings.cancelImg + '" border="0" /></a>\
									</div>\
	                    ...
	                    $(this).bind("rfuError", { 'action': settings.onError }, function (event, queueID, fileObj, errorObj) {
	                        if (event.data.action(event, queueID, fileObj, errorObj) !== false) {
	                            ...
	                            // remove error Items automatically (Leta)
	                            if (settings.RemoveErrorItems) {
	                                $("#" + $(this).attr('id') + queueID).hide();
	                            }
	                        }
	                    });
	        ...
	        fileUploadCancel: function (queueID, filename) {
	            $(this).each(function () {
	                document.getElementById($(this).attr('id') + 'Uploader').cancelFileUpload(queueID);
	            });

	            // Remove error items manully [Leta]
	            if (!settings.RemoveErrorItems) {
	                var fileObj = new Object();
	                fileObj.name = filename;
	                $("#" + $(this).attr('id') + queueID).hide();
	                settings.onCancel.call(this, null, queueID, fileObj, null);
	            }
	        },
